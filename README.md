...
- 总零色散波长不等于纯石英材料零点：D_wg 为负，把 ZDW 推向更长波长。
- 近截止宏弯损耗远大于短波侧；Petermann 与 Marcuse 光斑代入熔接公式不得给出同一损耗。
...
- `GET /api/example` — 预置 SMF-1310 算例
- `POST /api/mode` — NA、V、单模判定与三项色散（请求体即配置 JSON）
- `POST /api/splice` — 两根光纤的熔接重叠损耗与横向错位损耗

非法参数返回 `{"error":"…"}` 与 400。浏览器打开 http://localhost:8080。容器默认拉起同一服务。
