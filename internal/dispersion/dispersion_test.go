...
func TestZeroDispersionDiffersFromMaterialZero(t *testing.T) {
	mat, err := MaterialZeroOf(Silica(), 1.20, 1.40)
	if err != nil {
		t.Fatalf("material zero: %v", err)
	}
	tot, err := ZeroDispersionWavelength(smfCfg(), 1200, 1800)
	if err != nil {
		t.Fatalf("total zero: %v", err)
	}
	if math.Abs(tot-mat*1e3) < 15 {
		t.Errorf("total ZDW %v nm collapsed onto material %v nm; D_wg must shift it", tot, mat*1e3)
	}
	neg, err := WaveguideKeepsNegativeSign(smfCfg())
	if err != nil {
		t.Fatalf("sign: %v", err)
	}
	if !neg {
		t.Error("step-index D_wg must stay negative")
	}
}

func TestGermaniaMixShiftsMaterialZero(t *testing.T) {
	shift, err := MaterialZeroShiftNm(0.08)
	if err != nil {
		t.Fatalf("shift: %v", err)
	}
	if shift <= 5 {
		t.Errorf("8%% GeO2 should push material ZDW by >5 nm, got %v", shift)
	}
	dn, err := GermaniaRaisesIndex(1.31, 0.08)
	if err != nil {
		t.Fatalf("index: %v", err)
	}
	if dn <= 0 {
		t.Errorf("GeO2 must raise n, got %v", dn)
	}
	cfg := smfCfg()
	withProf, err := ComposeWithProfile(cfg, 0.04)
	if err != nil {
		t.Fatalf("profile: %v", err)
	}
	plain, err := TotalDispersionAt(cfg)
	if err != nil {
		t.Fatalf("plain: %v", err)
	}
	if math.Abs(withProf.DTotal-plain.DTotal) < 1e-4 {
		t.Errorf("Ge-doped profile left D_tot unchanged: %v vs %v", withProf.DTotal, plain.DTotal)
	}
}
