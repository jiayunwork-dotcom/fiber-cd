...
func TestBendLossExplodesNearCutoff(t *testing.T) {
	cfg := smfCfg()
	near := cfg.Clone()
	near.WavelengthNm = 1550
	far := cfg.Clone()
	far.WavelengthNm = 980
	const Rc = 0.008
	ln, err := MacrobendLossFor(near, Rc)
	if err != nil {
		t.Fatalf("near cutoff: %v", err)
	}
	lf, err := MacrobendLossFor(far, Rc)
	if err != nil {
		t.Fatalf("far from cutoff: %v", err)
	}
	if !(ln > lf*3) {
		t.Errorf("near-cutoff bend %v should greatly exceed far %v", ln, lf)
	}
	tight, err := MacrobendLossFor(cfg, 0.005)
	if err != nil {
		t.Fatalf("tight: %v", err)
	}
	loose, err := MacrobendLossFor(cfg, 0.02)
	if err != nil {
		t.Fatalf("loose: %v", err)
	}
	if !(tight > loose) {
		t.Errorf("tighter bend should lose more: %v vs %v", tight, loose)
	}
}

func TestLP11CutoffMatchesJ0(t *testing.T) {
	if math.Abs(LP11Cutoff().Root-CutoffV) > 1e-12 {
		t.Errorf("LP11 root %v vs CutoffV %v", LP11Cutoff().Root, CutoffV)
	}
	if math.Abs(CutoffVFromJ0()-CutoffV) > 0.002 {
		t.Errorf("J0 first root %v drifted from CutoffV %v", CutoffVFromJ0(), CutoffV)
	}
	cfg := smfCfg()
	na := NumericalAperture(cfg.N1, cfg.N2)
	lam, err := LP11CutoffWavelengthNm(cfg.CoreRadiusM(), na)
	if err != nil {
		t.Fatalf("LP11 λc: %v", err)
	}
	want := CutoffWavelengthNm(cfg.CoreRadiusM(), na)
	if math.Abs(lam-want) > 1e-6 {
		t.Errorf("LP11 cutoff %v nm, Analyze cutoff %v nm", lam, want)
	}
	if IsMultimodeAt(2.0) {
		t.Error("V=2.0 must stay single-mode")
	}
	if !IsMultimodeAt(3.0) {
		t.Error("V=3.0 must be multimode")
	}
}
